...
----------------------------------------------------------------------------*/
    void getCmdLine()
    {
        ...
        if (nbytesread > 0)
        {
            for (p = cmdline; p < cmdline + nbytesread; p++)
                if (*p == '\0')
                    *p = ' ';
...
----------------------------------------------------------------------------*/
    void AddSerLock(const char *szttyDevice, const char *devLCKfile, const long unsigned int PID, char *COMMAND, const int debug_flag)
    {
        ...
        do
        {
            ...
            errno = 0;
            if (flock(fileno(fdserlck), LOCK_SH | LOCK_NB) == 0)
                break; // Lock Acquired
            errno_save = errno;
...
----------------------------------------------------------------------------*/
    void *getPIDcmd(long unsigned int PID)
    {
        ...
        *cmdFilename = '\0';
        ...
        if ((fdcmd = open(cmdFilename, O_RDONLY)) < 0)
            return NULL;
...
----------------------------------------------------------------------------*/
    void lockSer(const char *szttyDevice, const long unsigned int PID, int debug_flag)
    {
        ...
        if (debug_flag)
            log_message(debug_flag, "Checking for lock");
        while (LckPID != PID && tv_diff(&tLockNow, &tLockStart) <= yLockWait * 1000000L)
        {
            ...
            { //fread OK
                ...
                if (LckPID != oldLckPID)
                {
                    log_message(debug_flag, "PID: %lu COMMAND: \"%s\" LckPID: %lu LckCOMMAND: \"%s\" LckPIDcommand \"%s\"%s", PID, COMMAND, LckPID, LckCOMMAND, LckPIDcommand, LckPID == PID ? " = me" : "");
                    oldLckPID = LckPID;
                }
        ...
        free(COMMAND);
        if (LckPID == PID)
            log_message(debug_flag, "Appears we got the lock.");
        if (LckPID != PID)
        {
            ClrSerLock(PID);
    ...
    {
        int mqtt_address_flag = 0;
        int mqtt_topic_flag = 0;
    ...
    time_t byte_timeout = -1;
        ...
        while ((c = getopt(argc, argv, "Q:F:QF:a:Ab:BcCd:D:efgij:lmM:nN:opP:qr:R:s:S:tTvw:W:xy:z:12")) != -1)
        {
            switch (c)
            {
            case 'Q':
        ...
        else
        {
            log_message(debug_flag, "Libmodbus context open (%d%s%d)",
                        baud_rate,
                        (parity == E_PARITY) ? EVEN_parity : (parity == N_PARITY) ? NONE_parity : ODD_parity,
                        stop_bits);
        }
    ...
    log_message(debug_flag, "New response timeout: %ds, %dus", timeout.tv_sec, timeout.tv_usec);
        ...
        if (volt_flag == 1)
        {
            ...
            read_count++;
            if (mqtt_address_flag == 1)
            {
                printf("SERVER MQTT IP=%s\n", mqtt_server_ip);

                if (mqtt_topic_flag == 1)
                {
                    printf("Topic MQTT=%s\n", mqtt_topic_root);
                }
